...
async function load(url, text) {
  if (url) {
    try {
      ...
      if (!fetchReponse.ok)
        throw new Error(
          '<h2 style="color:red;">There was Error fetching from link, Please check the link and try again.</h2>'
        );
...
function createIndex() {
  ...
  let index = `<div id="index">\n<ul id="index-list">\n`;
  for (let i = 1; i < indexHeadings.length; i++) {
    let headingType = parseInt(indexHeadings[i].id.charAt(1));
    index = index.concat(
      `<li class="index-li-h${headingType}"><a href="#${indexHeadings[i].id}">${indexHeadings[i].title}</a></li>\n`
    );
  }
...
function postProcess() {
  //--- add back escaped *
  //- In C and C++ * is used inside of code block as pointers
  // and we need to initially escape it so we don't create em or strong block in code.
  stringHTML = stringHTML.replaceAll("%2A", "*");
  ...
  //Put everything into html body.
  const htmlTemplateStart =
    '<!DOCTYPE html>\n<html lang="en">\n<head>\n<title></title>\n</head>\n<body>\n';
  const htmlTemplateEnd = "\n</body>\n</html>";
  //Add the whole processed Html into a container.
  stringHTML =
    htmlTemplateStart +
    '<div id="container">\n' +
    stringHTML +
  ...
  //Put everything other than the index in a div with id=content
  let contentMatch = matchAllRegex(stringHTML, regexJSON["content"]);
  for (let content of contentMatch) {
    stringHTML =
      stringHTML.slice(0, content.index) +
      '\n<div id="content">\n' +
      stringHTML.slice(content.index, content.index + content[0].length) +
      "\n</div>\n" +
      stringHTML.slice(content.index + content[0].length);
  }
}
...
function isTableRow(line) {
  ...
  let returnline = "<tr>";
  rowElements.forEach(
    (elem) => (returnline = returnline.concat(`\n\t<td>${elem}</td>`))
  );
...
function isTable(string) {
  ...
  while (matchTableDivison) {
    string =
      string.slice(0, matchTableDivison.index) +
...
function isCodeBlock(line) {
  ...
  if (count > 2) {
    ...
    while (lineStartWith[line[0]] !== "code") {
      Object.keys(htmlEsacpeCharacters).forEach(
        (key) => (line = line.replaceAll(key, htmlEsacpeCharacters[key]))
      );
...
fromTextButton.onclick = () => {
  ...
  load(undefined, text).then((result) => {
    ...
    // iframeResultPage.contentWindow.location.assign(iframeResultPage.contentWindow.location.ancestorOrigins[0]);
    // iframeResultPage.contentWindow.location.reload();
